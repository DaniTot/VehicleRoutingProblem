...
class VRP:
    ...
    def setup(self):
        ...
        # This constrain does not care about vehicles returning to the starting depot self.V[0]
        # So you can have a situation, where vehicle arrives,
        # but leaves once for the starting depot, and once for some other node
        for i in self.N:
            for k in self.K:
        ...
        #     for k in self.K:
        #         self.model.addConstr(quicksum(self.x[j, i, k] for j in self.V[:-1] if j != i) ==  # What arrives to i
        #                              quicksum(self.x[i, j, k] for j in self.V[:] if j != i),     # Must leave from i
        #                              name=f"Leave_{i}{k}")
        # Capacity constraint
        for k in self.K:
            self.model.addConstr(quicksum(self.q[j] * self.x[i, j, k] for i in self.V for j in self.N if i != j) <= self.Q,
                                 name=f"Capacity_{k}")
